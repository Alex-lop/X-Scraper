...
Checked: **2026-08-19**. Audit work began at
...
| Durable single jobs and atomic 2-25-source batches, priority/FIFO/source fairness, idempotency, restart recovery, progress gaps, cancellation, and per-source/auth cap 1 | `proven offline` | `tests/test_jobs.py`, storage/API/UI tests, and adversarial recovery exercise durable leases and terminal precedence. Default workers are 1, the user maximum is 2, and the code hard maximum is 4. These tests do not prove safe concurrent live-X collection. |
...
| One-/two-worker production-provider performance against a local Chromium fixture | `proven local Chromium` | The canonical macOS artifact records 4/4 correct jobs for each case, zero cleanup failures/orphan Chromium, and a 2.415x wall-time speedup (`3.111s` to `1.288s`) at about 385 MB additional peak process-tree RSS. It supports default 1/opt-in 2 only, not a higher live-X default. |

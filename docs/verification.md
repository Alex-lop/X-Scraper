...
| Recorded ordinary Python, JavaScript, lint, CLI, and wheel gates | `proven offline` | At dashboard checkpoint `71d42c5`, the normal full suite with local Chromium reported `229 passed, 32 skipped`; 31 skips are the gated capability lab and one is the opt-in browser matrix. Ruff, both help commands, wheel inspection/base-install behavior, and all `6/6` JavaScript tests passed locally. The fresh wheel excluded tests and the lab and guarded Textual behind `extra == "tui"`. |
...
| One-/two-worker production-reachable mixed-provider performance against local fixtures | `proven local Chromium` | The [2026-08-20 three-run artifact](benchmarks/reachable-mixed-provider-2026-08-20.json) uses the real route, SQLite, Playwright Browser, and synthetic official transport. Paired runs used alternating `AB/BA/AB` order and per-case RSS baselines; residual order and warm-cache bias remain. CPU excludes sampler-thread time and observer `ps` processes and includes observed descendant CPU without `RUSAGE_CHILDREN`. Medians were `1.066s` versus `0.591s` (`1.804x`); CPU fell, the two-worker baseline-adjusted RSS increment was `212,992` bytes higher, and SQLite fraction, backlog, correctness, cleanup, and zero-egress gates passed. This supports a global maximum two only; same-provider work remains serial and no live-X speed claim follows. |

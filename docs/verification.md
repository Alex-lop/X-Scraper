...
milestone was `a3196ca050da9b4e6f6255058719bfdacfec408e`; the latest hosted feature checkpoint
recorded here is `62ccc366906d15e086ba872d061165c835b1d201`. The previously recorded
capability-lab revision is `1bd21ea9d0c7766b9440b65891569260ed92e5f3`.
...
| Recorded ordinary Python, JavaScript, lint, CLI, and wheel gates | `proven offline` | At repair source checkpoint `74fe233`, the normal full suite with local Chromium reported `237 passed, 32 skipped`; 31 skips are the gated capability lab and one is the opt-in browser matrix. Ruff, both help commands, and all `6/6` JavaScript tests passed locally. Feature checkpoint `62ccc36` also passed hosted wheel inspection/base-install behavior; the fresh wheel excluded tests and the lab and guarded Textual behind `extra == "tui"`. |
| Recorded Linux Python 3.11-3.13 CI, local-Chromium job, and Linux no-egress capability-lab job | `proven offline` | [Run 32337098742](https://github.com/Alex-lop/X-Scraper/actions/runs/32337098742) was green at feature checkpoint `62ccc36`. It is checkpoint evidence, not a claim about later unpushed changes; no X credentials or live target were supplied. Earlier [run 32230574720](https://github.com/Alex-lop/X-Scraper/actions/runs/32230574720) passed the same chain at `1bd21ea`, and baseline [run 31074707953](https://github.com/Alex-lop/X-Scraper/actions/runs/31074707953) was green at the starting revision. |
...
| Optional terminal owner and attachable read-only monitor | `proven offline` | Textual 8.2.8 Pilot covers setup/capture/queue at normal, wide, and narrow sizes. Tests prove exact and paid approvals, expiry/edit invalidation, unknown-outcome refresh without resubmission, cancellation, stale reconnect, bounded sanitized rendering, auth cancellation, shared-client URL/response limits, main-thread owner lifecycle, and a monitor that creates neither SQLite nor a worker. Base-wheel tests fail cleanly before runtime state when Textual is absent. |
...
| One-/two-worker production-reachable mixed-provider performance against local fixtures | `proven local Chromium` | The [2026-08-20 three-run artifact](benchmarks/reachable-mixed-provider-2026-08-20.json) was recorded from tracked-clean revision `74fe2333ee44dd7dc26f4b9dad6c28bf59f16bd5` with its exact command. It uses the real route, SQLite, Playwright Browser, and synthetic official transport. Paired runs used alternating `AB/BA/AB` order and per-case RSS baselines; residual order and warm-cache bias remain. CPU excludes sampler-thread time and observer `ps` processes and includes observed descendant CPU without `RUSAGE_CHILDREN`. Medians were `1.194s` versus `0.591s` (`2.020x`); CPU grew about 4.2%, the two-worker baseline-adjusted RSS increment was `30,621,696` bytes higher, and SQLite fraction, backlog, correctness, cleanup, and zero-egress gates passed. This supports a global maximum two only; same-provider work remains serial and no live-X speed claim follows. |
| Historical isolated Browser-runtime matrix | `proven local Chromium` | The preserved [2026-08-19 artifact](benchmarks/queue-performance-2026-08-19.json) records the raw 2.415x result, but direct submission injected a unique synthetic auth key per Browser job and bypassed production admission. It proves isolated runtime cleanup only, not production Browser concurrency. |
| Production Browser projection, profile/Latest-search destination derivation, target/stall behavior, deduplication, drift telemetry, cancellation, and lifecycle cleanup | `proven local Chromium` | Five real-Chromium provider integration cases plus browser/hardening/adversarial tests use static, delayed, and virtualized sanitized fixtures. Expected navigation is fulfilled locally and unexpected requests abort. Page/context/browser closure is asserted; current X selectors, acceptance, ordering, and completeness are not. |
...
| Offline dashboard workflow, rendering, responsive layout, and basic interaction/accessibility state | `proven local Chromium` | An automated real-app path uses Flask, SQLite, `JobService`, a blocking local provider, and Chromium to prove preview, confirmation, 50% progress, cancellation, partial analysis filtering, and schema-v4 JSON export with every non-loopback request rejected. Earlier recorded QA covered no overflow, named controls, skip-link/focus, dynamic `aria-current`, the exact demo comparison, and 25 evidence cards. Two sanitized captures remain in [the demo record](demo.md); this is not a full accessibility audit. |
...
| Windows clean install, terminal use, or headed authentication | `not yet verified` | A PowerShell path is documented, but there is no Windows CI, terminal/Pilot platform record, or clean-platform record. POSIX permission evidence does not transfer to NTFS. |
...
also passed. At `c843690`, the then-current wheel had 24 members and 14 Python modules. The current
54-pin source tree builds a 26-member wheel with 16 Python modules, including the shared client and
terminal UI; its metadata guards Textual behind the `tui` extra, and a base-wheel smoke proves both
terminal commands fail cleanly without Textual or runtime state. The convenience extras install in
